...
Result HubReadDescriptor(struct UsbDevice *device) {
	...
	if (((struct HubDevice*)device->DriverData)->Descriptor == NULL &&
		(((struct HubDevice*)device->DriverData)->Descriptor = malloc(header.DescriptorLength)) == NULL) {
...
Result HubGetStatus(struct UsbDevice *device) {
	Result result = UsbControlMessage(
		device, 
	...
	if (result == OK && device->LastTransfer < sizeof(struct HubFullStatus)) {
...
Result HubPortGetStatus(struct UsbDevice *device, uint8_t port) {
	...
	if (result == OK && device->LastTransfer < sizeof(struct HubPortFullStatus)) {
...
Result HubAttach(struct UsbDevice *device, uint32_t interfaceNumber) {
	...
	if ((device->DriverData = malloc(sizeof(struct HubDevice))) == NULL) {
